...
void startup_sequence(int animation, int info) {
    ...
    if (animation == 1) stars_animations(clear_after);
    if (animation == 2) boot_animation(clear_after);
    if (animation == 3) glitch_animation(clear_after);
    if (animation == 4) rain_animation(clear_after);
    ...
    if (info == 4) {
        ...
        if (home) {
            snprintf(cmd, sizeof(cmd),
                "neofetch --source %s/.mysh_logo.txt --ascii_colors %d %d --colors %d 7 %d %d %d %d --color_blocks off",
                home,
                accent_color_term, accent_color_term,
...
void stars_animations(int clear_after) {
    ...
    if (clear_after) printf("\033[2J\033[3J\033[H");
}
    ...
    for (int f = 0; f < 5; f++) {
        ...
        for (int i = 0; glogo[i] != NULL; i++)
            printf("\033[%d;%dH\033[38;5;%dm%s\033[0m",
                logo_y + i, logo_x, stabilize_farben[f], glogo[i]);
        fflush(stdout);
        usleep(100000);
    }
    ...
    for (int frame = 0; frame < 40; frame++) {
        ...
        for (int col = 0; col < cols; col++) {
            for (int row = 0; row < rows; row++) {
                ...
                if (dist == 0) {
                    printf("\033[%d;%dH\033[38;5;%dm|\033[0m", row + 1, col + 1, accent_color);
