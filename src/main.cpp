  ...
  while (1) {
    if (interactive) {
      ...
      if (find_git_root(cwd, git_path)) {
        get_git_branch(git_path, branch, sizeof(branch));
        int clean = get_git_status(git_path);
        const char *status_icon = clean ? "✓" : "✗";
        snprintf(
            ...
            clean ? color_git_clean : color_git_dirty, branch, status_icon,
        ...
        snprintf(
            ...
            "\001\033[0m\002"
            " \001\033[36m\002❯\001\033[0m\002 ",
            color_folder, folder, color_folder, color_time, uhrzeit,
            color_time);
      }
    }
